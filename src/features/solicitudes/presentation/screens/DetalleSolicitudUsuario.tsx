...
const DetalleSolicitudUsuario: React.FC = () => {
  ...
  const enviarConsulta = () => {
    ...
    const codigo = obtenerId(solicitud);
    const asunto = `Consulta sobre solicitud ${codigo}`;
    const cuerpo = [
      `Hola, necesito ayuda con la solicitud ${codigo}.`,
      `Tramite: ${obtenerTramite(solicitud)}.`,
      "",
      "Mi consulta es:",
    ].join("\n");

    window.location.href = `mailto:atencionciudadana@santodomingo.cl?subject=${encodeURIComponent(
      asunto
  ...
  useEffect(() => {
    void cargarSolicitudEstable(false);

    const intervalo = window.setInterval(() => {
      void cargarSolicitudEstable(true);
    }, 10000);
  ...
  const documentosAdjuntos: DocumentoAdjunto[] = (() => {
  ...
  const documentosSolicitados: DocumentoSolicitud[] = (() => {
          ...
          <main className="detalle-main">
            ...
            <section className="comentario-funcionario-card">
              ...
              <div className="comentario-funcionario-body">
                <strong>Antecedentes del trámite</strong>

                <DatosTramite solicitud={solicitud} />
